...
def init_db():
    ...
    default_group = cursor.execute("""
        SELECT * FROM groups
        WHERE name = ?
    """, ("Вторник, Четверг. ТГАСУ",)).fetchone()
...
@app.route("/superadmin/merge-duplicate-default-groups")
@superadmin_required
def merge_duplicate_default_groups():
    db = get_db()
    cursor = db.cursor()

    group_name = "Вторник, Четверг. ТГАСУ"
    ...
    for duplicate in groups[1:]:
        ...
        duplicate_members = cursor.execute("""
            SELECT *
            FROM group_members
            WHERE group_id = ?
        ...
        for member in duplicate_members:
            exists = cursor.execute("""
                ...
                WHERE group_id = ?
                  AND user_id = ?
            """, (keep_id, member["user_id"])).fetchone()
        ...
        result.append(f"Удалён дубль id={duplicate_id}, данные перенесены в id={keep_id}")
